...
`best-practices`, `coverage`, `refactor`, `bug`, `retrospective`, `parity`) is set by an explicit spawn-context directive
...
`.agents/build/skills/blackhole/references/blackhole-state.md` for the atomic pattern, not
