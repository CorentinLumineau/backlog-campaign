...
`.claude/skills/blackhole/references/queue-dag.md` § `route` object — that table is the
...
Compute `route.confidence.{split,design,plan_mode,security,docs,brainstorm,analysis}`, each a
0-100 score, one per gated flag. You compute the score; you do not gate on it. The
cautious-default gating logic that consumes these scores already lives in `orchestrator.md` §
Route-derived dispatch — do not duplicate it here.
...
### needs_brainstorm classification (ADR-010 D1)

`route.needs_brainstorm` is content-derived, same as `docs_impact` above. Classify `true` when
the issue body **both** (a) lacks testable, machine-verifiable acceptance criteria, **and** (b)
lacks a concrete technical mechanism (no named files, APIs, or approach — pure idea-stage
prose). Classify `false` when either (a) or (b) is satisfied. The confidence-gate and voiding
rule that consume this classification are specified once, in `queue-dag.md` § `route` object —
not re-tabulated here.
...
`.claude/skills/blackhole/references/findings-ledger.md` § "Routing decision records"
for the `local_analyze` field shape; do not re-tabulate it here (`V-DRY-01`).

## Write protocol

Single-writer-orchestrator invariant
(`.claude/skills/blackhole/references/blackhole-state.md` § Single-writer invariant):
...
2. **`findings-ledger.json`** — append one `routing_decisions` row per
   `.claude/skills/blackhole/references/findings-ledger.md` § "Routing decision
...
full object (same shape as `.claude/skills/blackhole/references/findings-ledger.md` §
